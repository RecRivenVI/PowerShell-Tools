...
ForEach-Object -Parallel { 
    ...
    if ($using:ExistedTgzs.Name -contains ($json.LatestFullRom.filename -replace " ","")) {
        Write-Host "$($json.LatestFullRom.version)已存在|$($json.LatestFullRom.device)"
    } else {
        Write-Host ("$($json.LatestFullRom.version)已更新|$($json.LatestFullRom.device)|https://bkt-sgp-miui-ota-update-alisgp.oss-ap-southeast-1.aliyuncs.com/$($json.LatestFullRom.version)/$($json.LatestFullRom.filename)" -replace " ","")
    }
} -ThrottleLimit 8